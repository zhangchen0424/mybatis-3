...
public class Reflector {
  ...
  private void resolveGetterConflicts(Map<String, List<Method>> conflictingGetters) {
    ...
    for (Entry<String, List<Method>> entry : conflictingGetters.entrySet()) {
      ...
      for (Method candidate : entry.getValue()) {
        ...
        // 如果两个方法返回值类型相同，则判断是否是 布尔类型，如果不是 boolean 类型且返回值相同，抛出异常，因为这违反了 JavaBeans 规范。
        if (candidateType.equals(winnerType)) {
          if (!boolean.class.equals(candidateType)) {
            throw new ReflectionException(
             ...
             // 如果是 boolean 类型且候选方法名以 "is" 开头，则选择该方法（优先选择 isXxx 形式而非 getXxx 形式）
          } else if (candidate.getName().startsWith("is")) {
            winner = candidate;
          }
          //候选者类型是胜出者父类，则还保持原来的胜出者
        } else if (candidateType.isAssignableFrom(winnerType)) {
          // OK getter type is descendant

            //胜出者是候选者父类，选择候选者（因为子类可以修改放大返回值。例如，父类的一个方法的返回值为 List ，子类对该方法的返回值可以覆写为 ArrayList）
  ...
  private void addSetMethods(Class<?> cls) {
    ...
    for (Method method : methods) {
      String name = method.getName();
      // 以 set 方法名开头，说明是 setting 方法,且参数数量为1
      if (name.startsWith("set") && name.length() > 3) {
        if (method.getParameterTypes().length == 1) {
            // 获得属性
          name = PropertyNamer.methodToProperty(name);
          //添加conflictingSetters
  ...
  private void addFields(Class<?> clazz) {
    ...
    for (Field field : fields) {
        //检查该字段是否已经有对应的setter方法。
      if (!setMethods.containsKey(field.getName())) {
        // issue #379 - removed the check for final because JDK 1.5 allows
        // modification of final fields through reflection (JSR-133). (JGB)
        // pr #16 - final static can only be set by the classloader
          //判断字段是否为final static类型，只有不是final static的字段才会添加setter访问器。
        int modifiers = field.getModifiers();
        if (!(Modifier.isFinal(modifiers) && Modifier.isStatic(modifiers))) {
            //为字段添加Setter访问器（SetFieldInvoker）。
          addSetField(field);
        }
      }
      //检查该字段是否已经有对应的getter方法。
      if (!getMethods.containsKey(field.getName())) {
          //为字段添加Getter访问器（GetFieldInvoker）。
        addGetField(field);
      }
    }
    //如果当前类有父类且不是Object类，则递归处理父类的字段。
  ...
  private Method[] getClassMethods(Class<?> cls) {
      // 每个方法签名与该方法的映射key是方法签名，value是方法
  ...
  private void addUniqueMethods(Map<String, Method> uniqueMethods, Method[] methods) {
    for (Method currentMethod : methods) {
        // <1> 忽略桥接方法 https://www.zhihu.com/question/54895701/answer/141623158
      if (!currentMethod.isBridge()) {
        ...
        // overridden a method
          // 当signature在 uniqueMethods 中不存在时，进行添加
