    ...
    public SqlCommand(Configuration configuration, Class<?> mapperInterface, Method method) {
      ...
      if (ms == null) {
        ...
        if(method.getAnnotation(Flush.class) != null){
          ...
          // <3> 找到 MappedStatement,对应 MappedStatement#getId() 方法获得的标识。实际上，就是 ${NAMESPACE_NAME}.${语句_ID}， 例如："org.apache.ibatis.autoconstructor.AutoConstructorMapper.getSubject2"
        name = ms.getId();
    ...
    private Integer getUniqueParamIndex(Method method, Class<?> paramType) {
      ...
      for (int i = 0; i < argTypes.length; i++) {
        if (paramType.isAssignableFrom(argTypes[i])) {// 类型符合
          if (index == null) {// 获得第一次的位置
            index = i;
          } else {// 如果重复类型了，则抛出 BindingException 异常
